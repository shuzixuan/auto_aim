...
uint8_t verify_crc8_checksum(uint8_t* pchMessage, uint16_t dwLength) {
    ...
    if ((pchMessage == 0) || (dwLength <= 2)) {
        ...
        ucExpected = generate_crc8_checksum(pchMessage, dwLength - 1, CRC8_INIT);
        return (ucExpected == pchMessage[dwLength - 1]);
    }
}

/**
 * @brief  append CRC8 checksum to the end of data
...
void append_crc8_checksum(uint8_t* pchMessage, uint16_t dwLength) {
    ...
    if ((pchMessage == 0) || (dwLength <= 2))
        return;
    ucCRC                    = generate_crc8_checksum((uint8_t*)pchMessage, dwLength - 1, CRC8_INIT);
    pchMessage[dwLength - 1] = ucCRC;
}

/**
 * @brief   generate CRC16 checksum
 * @param   pchMessage  Data to check
 * @param   dwLength    Stream length
 * @param   ucCRC8      initialized checksum
...
uint8_t verify_crc16_checksum(uint8_t* pchMessage, uint32_t dwLength) {
    ...
    if ((pchMessage == NULL) || (dwLength <= 2)) {
        ...
        wExpected = generate_crc16_checksum(pchMessage, dwLength - 2, CRC_INIT);
...
void append_crc16_checksum(uint8_t* pchMessage, uint32_t dwLength) {
    ...
    wCRC                     = generate_crc16_checksum((uint8_t*)pchMessage, dwLength - 2, CRC_INIT);
